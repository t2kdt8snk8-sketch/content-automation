...
카테고리:
- 감상 원재료: 알고 나면 곡이나 앨범이 다르게 보이는 정보. 예시: 아티스트 인터뷰에서 밝힌 작곡·작사 배경, 평론가·음악 저널리스트의 곡 분석, 앨범 해설지(라이너 노츠) 내용, 앨범 커버의 기획 의도, 뮤직비디오 감독 발언·연출 의도, 팬 커뮤니티가 발견한 샘플 원곡·숨겨진 레퍼런스, 특정 가사의 의미·해석. 이와 비슷하게 "알고 나면 감상이 달라지는" 내용이면 모두 해당됩니다.
...
가사 제한: 가사 해석·분석 항목은 감상 원재료 전체 중 최대 1개. 나머지는 인터뷰, 평론, 앨범 해설 등 다른 출처에서 채우세요.
...
export function buildTrackDetailUserPrompt(input: {
  ...
  return [
    ...
    ...(input.externalContext
      ...
      : []),
    "",
    "각 곡마다 감상 원재료 8개, 흥미 TMI 2개, 총 10개 포인트를 수집해줘.",
    "가사 해석·분석 항목은 감상 원재료 8개 중 최대 1개. 나머지 7개는 인터뷰, 평론, 앨범 해설, 뮤직비디오 연출 의도 등 다른 출처에서 가져와.",
    "반드시 순수 JSON만 반환해. 마크다운 코드펜스, 설명 문장, 주석 넣지 마.",
    "모든 문자열은 반드시 큰따옴표를 사용해.",
