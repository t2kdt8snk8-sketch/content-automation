...
const FIXED_AGENT_ENTRIES: Array<[string, number]> = [
  ['research_agent', 1],
  ['copy_agent', 2],
  ['script_agent', 3],
  ['image_prompt_agent', 4],
  ['strategy_agent', 5],
];
const FIXED_AGENT_IDS = new Set(FIXED_AGENT_ENTRIES.map(([, id]) => id));
const agentIdMap = new Map<string, number>(FIXED_AGENT_ENTRIES);
let nextAgentId = 6;
...
export function OfficeCanvas({ lastEvent, onOverlayOpen }: OfficeCanvasProps) {
  ...
  useEffect(() => {
    ...
    const stop = startGameLoop(canvas, {
      ...
      render: (ctx) => {
        ...
        const agentRankById: Record<number, string> = {};
        for (const [agentName, id] of agentIdMap.entries()) {
          const rank = (AGENT_DISPLAY_NAMES as Record<string, string>)[agentName];
          if (!rank) continue;
          agentRankById[id] = rank;
        }
  ...
  useEffect(() => {
    ...
    if (lastEvent.type === 'agent_started') {
      ...
      if (id !== undefined && !FIXED_AGENT_IDS.has(id)) state.removeAgent(id);
      ...
      for (const id of agentIdMap.values()) {
        if (FIXED_AGENT_IDS.has(id)) continue;
        state.removeAgent(id);
      }
    }
  }, [lastEvent]);
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
      <canvas
        ...
        onMouseMove={(e) => {
          ...
          if (editorEnabled) {
            updateHoverStatus(hit);
            return;
          }

          const state = stateRef.current;
          if (!state) return;
          const charId = state.getCharacterAt(hit.worldX, hit.worldY);
          const furnitureUid = state.getFurnitureAt(hit.worldX, hit.worldY);
          e.currentTarget.style.cursor = (charId !== null) || !!furnitureUid ? 'pointer' : 'default';
        }}
        onClick={(e) => {
          const hit = toTileHit(e);
          if (!hit) return;
          ...
          const furnitureUid = state.getFurnitureAt(hit.worldX, hit.worldY);
          if (!furnitureUid) return;

          const furniture = state.getPlacedFurnitureByUid(furnitureUid);
          if (!furniture) return;

          const normalizedType = normalizeAssetType(furniture.type);
          if (normalizedType === 'SLICE_54_1') {
            setShowPcMenu(true);
            return;
          }
