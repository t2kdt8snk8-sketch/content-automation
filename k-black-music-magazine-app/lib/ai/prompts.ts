/*
검색 전략 (비활성 — 필요 시 프롬프트에 복원):
1차 검색: 반드시 아래 도메인에서 우선 검색하세요. 검색 쿼리에 site: 연산자를 포함하여 해당 도메인에서만 결과를 가져올 것.
  - site:whosampled.com — 샘플링 원곡, 오마주 정보
  - site:genius.com — 가사 주석, 자전적 맥락, 아티스트 의도
  - site:discogs.com — 앨범/트랙 크레딧, 발매 정보
  - site:soundonsound.com — 프로덕션 기법, 레코딩 비하인드
  - site:soultracks.com — 곡·앨범 리뷰, 감상 분석
2차 검색: 위 도메인에서 충분한 정보를 찾은 후, 추가로 일반 웹 검색을 1회 실행하여 놓친 감상 원재료를 보완하세요.
*/
...
카테고리:
- 감상 원재료: 알고 나면 곡이나 앨범이 다르게 보이는 정보. 예시: 샘플링 원곡 정보(어디서 뭘 가져왔는지), 자전적 맥락(실화 기반 여부·누구를/무엇을 향한 곡인지), 의도적 오마주·레퍼런스(선배 아티스트나 다른 곡을 향한 참조), 앨범 커버가 담은 상징, 뮤직비디오 연출 의도. 이와 비슷하게 "알고 나면 감상이 달라지는" 내용이면 모두 해당됩니다.
...
가사 제한: 감상 원재료 전체에서 가사 관련 항목(가사 해석, 가사 의미, 특정 가사 인용 등)은 반드시 1개 이하로만 포함하세요. 2개 이상 절대 금지.
...
export function buildTrackDetailUserPrompt(input: {
  ...
  return [
    ...
    "가사 관련 항목(가사 해석, 가사 의미 등)은 감상 원재료 8개 중 최대 1개만 포함할 것.",
    "반드시 순수 JSON만 반환해. 마크다운 코드펜스, 설명 문장, 주석 넣지 마.",
    "모든 문자열은 반드시 큰따옴표를 사용해.",
